...
import pro.handlers.OpenHandler;
import pro.model.Media;
import pro.model.MediaType;
...
public class MediaListPart {
	static class MediaCell extends ListCell<Media> {
		...
		protected void updateItem(Media item, boolean empty) {
			if(  ! empty && item != null ) {
				...
				switch (item.getType()) {
				case MOVIE:
					setGraphic(new ImageView("platform:/plugin/pro/icons/kaffeine.png"));
					break;
				case PICTURE:
					setGraphic(new ImageView("platform:/plugin/pro/icons/games-config-background.png"));
					break;
				default:
					setGraphic(new ImageView("platform:/plugin/pro/icons/player-volume.png"));
					break;
				}				
			}
	...
	void handleOpen() {
		...
		if( m.getType() == MediaType.MOVIE ) {
			p.setIconURI("platform:/plugin/pro/icons/22/kaffeine.png");	
		} else if( m.getType() == MediaType.PICTURE ) {
			p.setIconURI("platform:/plugin/pro/icons/22/games-config-background.png");
		} else {
			p.setIconURI("platform:/plugin/pro/icons/22/player-volume.png");
		}
		
		p.setContributionURI("bundleclass://pro/pro.parts.MediaPart");
	...
	private static ObservableList<Media> createList() {
		...
		l.add(new Media(MediaType.PICTURE, "Desert",     "platform:/plugin/pro/icons/resources/pics/pic1.jpg"));
		l.add(new Media(MediaType.PICTURE, "Lighthouse", "platform:/plugin/pro/icons/resources/pics/pic2.jpg"));
		l.add(new Media(MediaType.MOVIE,   "Grog",       "platform:/plugin/pro/icons/resources/movs/mov1.flv"));
